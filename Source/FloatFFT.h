...
#define TWO_PI 6.28318530717958623199592693708837032
...
public:
	...
	FloatFFT(int size): n(size) {
		...
		const bool isPowerOfTwo = !(size == 0) && !(size & (size - 1));
		...
		if(isPowerOfTwo) {
			...
			if(getReminder(n, factors) >= 211) {
				plan = BLUESTEIN;
				nBluestein = nextPowTwo(n * 2 - 1);
				bk1 = new float[2 * nBluestein];
				bk2 = new float[2 * nBluestein];
				ip = new int[2 + round(ceil(2 + (1 << (int)(log(nBluestein + 0.5)/log(2)) / 2)))];
				w = new float[nBluestein];
...
private:
	...
	int getReminder(int n, int factors[]) {
		int reminder = n;
		
		for(int i = 0; i < 4 && reminder != 1; ++i) {
	...
	int nextPowTwo(const int x) const {
		const int next = pow(2, ceil(log(x)/log(2)));
		return next;
	};
	...
	void passfg(int nac[], int ido, int ip, int l1, int idl1, float in[], int in_off, float out[], int out_off, int offset, int isign) {
		...
		int outindex = out_off;
		int inindex = in_off;
		for(int i = 0; i < idl1 - 1; ++i) {
			in[inindex] = out[outindex];
			++outindex;
			++inindex;
		}
	...
	void bluestein_complex(float a[], int offa, int isign) {
		float ak[2 * nBluestein];
			...
			cftbsub(2 * nBluestein, ak, ip, nw, w);
			...
			cftfsub(2 * nBluestein, ak, ip, nw, w);
			if (isign > 0) {
				for (int i = 0; i < n; i++) {
					int idx1 = 2 * i;
					int idx2 = idx1 + 1;
