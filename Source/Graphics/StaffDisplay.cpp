...
{
	prevActivePitches.ensureStorageAllocated(NUMBER_OF_VOICES);
...
void StaffDisplay::drawPitches(Array<int> activePitches, Graphics& g)
{
	...
	if(activePitches.isEmpty() == false && activePitches != prevActivePitches)
	{
		yCoordsOfActiveNotes.clearQuick();
	...
	prevActivePitches = activePitches;
};
