...
#  imogen_build_Android.sh :	  This script builds an Android version of Imogen.
...
if [[ ${#@} -ne 0 ]] && ( [[ ${@#"--help"} = "" ]] || [[ ${@#"-h"} = "" ]] ); then
 	printf "\n \t \v IMOGEN IOS BUILD SCRIPT \n USAGE: \n \n"
  	printf "Simply execute this script with no flags or arguments to build a default release confguration of Imogen in its Standalone format, cross-compiled for Android. \n"
  	exit 0;
fi
...
# configure CMake
printf "\n \t \v Configuring CMake... \n \n"
cmake -DCMAKE_BUILD_TYPE=release -B Builds/android_Build -DImogen_unitTesting=FALSE -Dbv_formats=Standalone -Dbv_alwaysForceCacheInits=TRUE .


# execute build
printf "\n \t \v Building Imogen... \n \n"
cmake --build Builds/android_Build --target Imogen_Standalone --config Release
